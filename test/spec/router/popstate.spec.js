...
define(function (require) {
    ...
    var controller = require('m-pjax/router/popstate');
    ...
    describe('Popstate Controller', function () {
        ...
        describe('init/dipose', function () {
            ...
            it('should apply handler with current path', function (done) {
                ...
                var path = args[0];
                var url = args[1];
                expect(path).toEqual('/test/runner.html');
                expect(url.query).toEqual('name=treelite&w=1');
            ...
            it('should monitor popstate', function (done) {
                var fn = jasmine.createSpy('fn');
        ...
        describe('redirect', function () {
            ...
            it('fire the handler with URL param', function (done) {
                ...
                setTimeout(function () {
                    var url = handler.calls.argsFor(0)[1];
                    expect(url.path).toEqual('/abc');
                    expect(url.getQuery()).toEqual({name: 'treelite'});

                    controller.redirect('/bbb', {query: 'abc'});
                    setTimeout(function () {
                        var url = handler.calls.argsFor(1)[1];
                        expect(url.path).toEqual('/bbb');
            ...
            it('fire the handler with options', function (done) {
                ...
                setTimeout(function () {
                    var url = handler.calls.argsFor(0)[1];
                    var options = handler.calls.argsFor(0)[2];
                    expect(url.path + '?' + url.query).toEqual('/abc?name=treelite');
